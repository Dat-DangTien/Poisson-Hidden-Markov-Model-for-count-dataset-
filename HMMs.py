...
class PHMM:
    ...
    def gen_seq(self, n=100):
        ...
        state = np.random.choice(a=self.nstates, p=np.exp(self.delta))#TẠO RA 1  GIÁ TRỊ LÀ NGUYÊN TỪ 0 ĐẾN A-1
    ...
    float array array - a nested sequence, where the subsequence at index i, j
                        ...
                        in the sequence.
    """
    def forward_lprobs(self, seq):#seq nhu input trên bảo là chỉ là 1 dãy thôi, chứ k phải dãy của dãy
        seterr(divide='ignore')
        g_1 = [self._sp_lpmf(self.lambdas[i], seq[0]) for i in range(self.nstates)]# trả về 1 array với mỗi phần tử là 
        #log của xác suất thời điểm đầu tiên ob tại seq[0] khi biết tham số Poisson là lambdas[i]
        g_1 = np.add(self.delta, g_1)#log (tích pi_0 x xs thời điểm đầu tại seq[0])
        glst = [g_1]
    ...
    def forward_lprob(self, seq):
        glst = self.forward_lprobs(seq)#là ma trận với mỗi hàng là ứng với 1 giá trị của t
        return logsumexp(glst[-1])#log(xich ma từ i đến N của alpha_T^(i))
    ...
    def forward_backward(self, seq):
        fprobs = self.forward_lprobs(seq)#cái forward_lprobs là ma trận có từng phần tử là log alpha(t) với t từ 0 đến T, i tu 1 den N
        ...
        return np.exp(norm_probs) # là ma trận các gamma_t^(i) với biến chạy là t và i, và hàng là đại diện cho biến i, tức ngược với hình
        # vẽ trong video trên youtube
    ...
    def log_likelihood(self, seqlst):
        probs = list(map(self.forward_lprob, seqlst))#vẫn đúng vì đây là lấy từng phần tử 
        return np.sum(probs)#bài toán 1 của HMMs nhưng đây là cho vô seqlst , coi seqlst như 1 dãy dài, còn của mk chỉ cần 1 seq là đc
    ...
    def viterbi(self, seq):
        ...
        for i in range(len(seq) - 1, 0, -1):#tu t=T tro xuong, tu trang thai cuoi
            ...
            for j in range(self.nstates):#for i tu 1 den N
                all_v_ij = []
                for k in range(self.nstates):#_sp_lmf là hàm tính xác suất 
                    temp = self.theta[j, k] + self._sp_lpmf(self.lambdas[k], seq[i])# là cái 
    ...
    """
    def baum_welch(self, seqlst, max_iter=200):#seqlst tức là list các seq
    #vấn đề trong file example.py: theo lý thuyết thường là chỉ cần truyền vào 1 chuỗi observation data để thuật toán baum-welch
    #luyện, tưởng chừng 1 chuỗi ra 1 bộ tham số, v mà ở đây seqlst gồm tần 20 chuỗi => mà kết quả chỉ ra 1 bọ tham sso nên có thể trong 
    #code này thì sử dụng nhiều chuỗi, ==> nếu dùng chuỗi từ data thì sẽ không phải code này được, vậy nên hiểu code này cái 
    #CHỐT 1 PHẦN: viterbi thì chỉ cần 1 seq còn baum trong đây cần lst cơ
            ...
            for i in range(len(trans)):
                for j in range(len(trans[i])):
                    if np.isneginf(trans[i, j]) or np.isneginf(prev_trans[i, j]):
                        ...
                        d = abs(trans[i, j] - prev_trans[i, j])# là log(thương 2 phần từ a_ij của ma trận ở vòng lặp trước và sau)
                        diff.append(d)
                        bools.append(d <= self.conv)
            for i in range(len(lambdalst)):
                d = abs(lambdalst[i] - prev_lambdalst[i])#là log(thương 2 tham số lambda của pp Poisson ở vòng lặp trước và sau)
                diff.append(d)
                bools.append(d <= self.conv)
            print("Difference: ", sum(diff))# phần này chắc là ĐẠI DIỆN log_likelihood function của 2 vòng lặp trừ đi nhau CHỨ CT KHÔNG 
            #GIỐNG HẲN NHƯ VẬY
            if ll:
                print("Log-Likelihood:", self.log_likelihood(seqlst))
            return all(bools)# chỉ khi mỗi phần tử trừ đi nhau đều nhỏ hơn cái chuẩn so sánh thì mới True hết, thì khi ý 
            #KẾT QUẢ bools mới là True hết với mọi phần tử thì mới là hội tụ

        while not assess_convergence() and itr < max_iter:
            prev_trans = trans#Gán 2 giá trị vừa lần trước là giá trị trước 
            prev_lambdalst = lambdalst#  để cập nhật cho giá trị vòng hiện tại
            trans_lst = []
            #seqs=seqlst
            seqs = [d for sub in seqlst for d in sub]#in ra seqs là chuỗi gồm 2000 phần tử với file example.py
            #print(len(seqs))
            rs = []#phần tạo nên sự khác biệt khi mới sử dụng từng chuỗi seq trong seqlst
            for seq, k in zip(seqlst, range(len(seqlst))):#zip(bien1,bien2) là đưa về dạng gồm các tuple(bien1[i],bien2[i]), để 
            #chuyển thành list of tuple thì cần đặt zip trong hàm đó là được 
                flst = self.backward_lprobs(seq)#tính ma trận beta_t^(i) với mỗi hàng là theo i
                rlst = []
                r_1_hat = np.add(self.delta, flst[0])#là log(tich của phân phối ban đầu với beta_0^(i) với biến chạy i)
                r_1_sum = logsumexp(r_1_hat)#log(xichma của i chạy từ 1 đến N của pi_i x beta_0^(i))
                r1 = list(map(lambda r: r - r_1_sum, r_1_hat))#ghi ra giấy cthuc r
                rlst.append(r1)
                tlst = []
                for i in range(1, len(seq)):
                    t_i_hat = []
            ...
            r1s = [d[0] for d in rs]#1 dãy gồm các rs[i][0], dường như rs là array array array= array matrix, mỗi seq thì rs là 1 matrix
            new_delta = []
